...
function escribir(results) {
	...
	if (parseInt(results.totalScore) >= 378) {
		explicacion.innerHTML =
			"Tu nivel de madurez digital se encuentra bajo. La forma de relacionarse con los clientes está cambiando, y no abordar la transformación digital de tu negocio es una barrera importante a tu desarrollo empresarial… ¡Es el momento de comenzar! En este nivel de madurez, la dimensión no sólo está completamente desarrollada, sino que la empresa se encuentra en la capacidad de influir sobre nuevos conocimientos referente a ella y puede ser un multiplicador de sus prácticas a otros.";
	} else if (parseInt(results.totalScore) >= 630) {
		explicacion.innerHTML =
			"Estás en un nivel medio de madurez digital. Te encuentras en pleno proceso de transición entre el comercio convencional y el digital… ¡Es el momento de seguir creciendo!  en este nivel de madurez las dimensiones se encuentran en una etapa desarrollada y presentan características más allá de lo que se espera o se encuentra en el mercado.";
	} else if (parseInt(results.totalScore) >= 945) {
		explicacion.innerHTML =
			"Estás a punto de alcanzar el máximo nivel de digitalización. En este momento, tu único propósito debe ser mejorar… ¡La digitalización de tu comercio no tiene límites! No sólo son básicas, sino que se ha adquirido una consciencia de sí mismas y empiezan una evolución. Se ha habilitado la compatibilidad con lectores de pantalla.";
	}
